...
base_url = 'http://35.159.155.184:8080'
...
for product, post in posts:
    ...
    response_product = requests.post('{}/product'.format(base_url), json=product)

    print(response_product.text)

    product_new = response_product.json()

    response_post = requests.post('{}/post'.format(base_url), json=post)
    ...
    image = requests.post('{}/image'.format(base_url), json=image).json()
    image_new = requests.post('{}/image/{}'.format(base_url, image['id']), files=files).json()

    requests.post('{}/post/{}/product'.format(base_url, post_new['id']), params={'productId': product_new['id']})
    requests.post('{}/post/{}/image'.format(base_url, post_new['id']), params={'imageId': image_new['id']})
