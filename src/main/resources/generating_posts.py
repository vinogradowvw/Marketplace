...
for product, post in posts:
    ...
    product = requests.post('http://localhost:8080/product', json=product).json()
    post = requests.post('http://localhost:8080/post', json=post).json()
    print(post)
    ...
    requests.post('http://localhost:8080/post/{}/product'.format(post['id']), json={'productId': product['id']})

    requests.post('http://localhost:8080/post/{}/image'.format(post['id']), json={'imageId': image['id']})
